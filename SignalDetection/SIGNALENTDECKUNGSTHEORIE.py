...
import os
import random
from psychopy import locale_setup, gui, visual, core, data, event, logging, clock, sound
...
#
#If Automatic is ticket in the GUI the experiment will consist of a series of blocks where the
# order of signal intensities is set in this array
list_of_intensities_automatic = [4,10,5,8,6,11,1,9,3,7,2]

#
#list_of_intensities_automatic = [9,6,12,3]


#[2.5,2.6,3.8,5,6,7,8.2,9.7]
###### STOP #######
...
print(variables.experiment_type)
######
...
filename = _thisDir + os.sep + u'data/'+expInfo['participant']+'/%s_%s_%s_%s' % (expInfo['participant'], expInfo['session'], exp_name, expInfo['date'])
...
window = visual.Window(
    color=[0, 0, 0],
    fullscr=False,
    size=[variables.screensize_x, variables.screensize_y], #size = [1440, 960],
    units='pix')
...
if not os.path.exists(_thisDir + os.sep + u'data/' + expInfo['participant'] + '/Fotos'):
    os.mkdir(_thisDir + os.sep + u'data/' + expInfo['participant'] + '/Fotos')
picture_location = _thisDir + os.sep + u'data/' + expInfo['participant'] + '/Fotos/%s_%s_%s' % (
        expInfo['participant'], exp_name, expInfo['date'])
#
...
if variables.automatic == True:
    new_signal_intensity = list_of_intensities_automatic[index]
    image_factory.variables.signal_intensity = new_signal_intensity
    image_factory.refresh_signal_intensity()
    variables.trial_blocks = len(list_of_intensities_automatic)
...
display_answer_y = visual.TextStim(window, "1" , pos=[0, 50])
display_answer_n = visual.TextStim(window, "2" , pos=[0, 50])
...
if variables.random_intensity:
    ...
    list_of_intensities = list_of_intensities_automatic
...
#        while trial < max_trial and trial_blocks < variables.trial_blocks:
        ...
        #            for thisTrial in trials:
                    ...
                    print(1, response_test_person)
                    if (exp_name == "Yes-No Task"or exp_name == "2IFCnew"):
                        print(2, response_test_person)
                    ...
                    if (exp_name == "2IFCnew"):
                        ...
                        trials_loop.addData('response_key2Interval_2IFCnew', second_key)
...
#                    trials_loop.addData('signal_picture', save_pic)
                    if len(response) == 1:
                        ...
                        trials_loop.addData('response', response[0])
                        if exp_name == "Yes-No Task":
                            trials_loop.addData('Subject_Action', response[1])
                            trials_loop.addData('Stimulus_Present', response[2])
                    ...
                    if exp_name == "2IFC" or exp_name == "2IFCnew":
                        ...
                        if exp_name=="2IFCnew":
                            trials_loop.addData('first_answer', first_answer)
                            trials_loop.addData('second_answer', second_answer)
                        if first_has_stim:
                            trials_loop.addData('stimulus_pos', 1)
                        else:
                            trials_loop.addData('stimulus_pos', 2)
                        if exp_name == "2IFC":
                            trials_loop.addData('button_pressed', button_pressed)
#                    if exp_name == "2IFC":
#                        trials_loop.addData('signal_on_stimuluspos2', not first_has_stim)
                    else:
                    ...
                    # random Intensity 
                    trials_loop.addData('random_intensity', variables.random_intensity)
                ...
                ### ändern der random intensitäten
                if variables.random_intensity:
                    new_signal_intensity = random.choice(list_of_intensities)
                    image_factory.variables.signal_intensity = new_signal_intensity
                    image_factory.refresh_signal_intensity()
...
#                    draw_component.setAutoDraw(True)
                ...
                # this component gets an n of "1" for yes or "2" for no
                ...
                if i < len(
                    ...
                    if not blocked:
                        ...
                        if exp_name == "2IFCnew":
                                ...
                                display_answer_locked = display_answer_pending
                    ...
                    if answer_given == False:
                        ...
                        if event.getKeys(keyList=["2"]):
                            ...
                            print(response_evaluation.get_answer_yes_no(
                                False, has_stim))
                            ...
                            if exp_name == "2IFCnew":
                                ...
                                draw_component = display_answer_locked
                                draw_component.setAutoDraw(True)
                                #for locking the answer but still wait
                                answer_given = True 
...
#                            right = sound.backend_sounddevice.SoundDeviceSound(261.63,0.2)
                            sound.Sound("Correct.wav").play()
...
#                            wrongsound = sound.backend_sounddevice.SoundDeviceSound(277.18,0.2)
                            sound.Sound("Wrong.wav").play()
#                            wrongsound.play()
...
#                            frame_remains = frame_remains - (variables.time_answer /2)
                    ...
                    if response_test_person != 0:
                        if event.getKeys(keyList=["2"]): #answer = No stimulus seen
    #                            answer_given = True 
                            no_answer = False
    ...
    #                            blocked = False 
        ...
        #                        if response_test_person == 1 or response_test_person == 3:
                        ...
                        if event.getKeys(keyList=["1"]): #1 = stimulus was seen
                            # "response_evaluation.get_answer_yes_no() evaluates the answer
                            # of the tested person and gives back 1-4 (hit, miss, correct rejection,
                            # false alarm), which is saved in "response_test_person"
    #                            answer_given = True 
...
#                    print(answer2IFCnew)
                    ...
                    if frame_switch == True:
                        ...
                        if trial_clock.getTime() > frame_remainsswitch:
                            # response_test_person = 0 for no answer
    #                        response_test_person_second = 0
                            i = i + 1
                            blocked = False
...
#                                response_test_person_second = 2
                        ...
                        #### minimaske
                if i < len(
                        variables.trial_composition) and variables.trial_composition[i] == 14:
    
                    # initialize routine for trial-component
                    if not blocked:
                        frame_remains = trial_clock.getTime() + variables.minimask - \
                            window.monitorFramePeriod * 0.75
                        blocked = True
...
#            thisExp.nextEntry()
        ...
        # else (no testtrials (left))
            ...
            if variables.random_intensity == False :
                print (variables.signal_intensity)
            print(
                "%i/%i, %g%%" %
                (correct,
                 variables.number_of_trials,
...
#            print(trial_blocks)
            ...
            countdown_clock = core.CountdownTimer(4.5)
            ...
            if trial_blocks + 1 < variables.trial_blocks:
                block_inst.draw()
                correctAnswers = "%g%% Korrekt"%(correct /variables.number_of_trials * 100)
                display_score = visual.TextStim(window, correctAnswers , pos=[0, 150])
                display_score.draw()
                if variables.random_intensity == False :
                    
                    signal_display = " Signalstärke %i"%(variables.signal_intensity)
                    display_signal_intensity = visual.TextStim(window, signal_display , pos=[0, 200])
                    display_signal_intensity.draw()
                window.flip()
                event.waitKeys(keyList=["w"])
