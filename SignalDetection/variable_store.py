...
class VarStore:
    ...
    def set_variables(self):
        ...
        self.number_of_trials = self.within_range(
            self.gui.gui_input_var.data[0], 1, 1000)
        # 1-100
        ...
        self.signal_intensity = self.within_range(
            self.gui.gui_input_var.data[12], -100, 100)
        # 50-200
        ...
        self.automatic = self.gui.gui_input_var.data[18]


        #create a new folder for the subject
        if not os.path.exists(self.gui.this_dir+os.sep + u'data/' + self.name_testperson):
            os.mkdir(self.gui.this_dir+os.sep + u'data/' + self.name_testperson)
        self.data_path = self.gui.this_dir + os.sep + u'data/' + self.name_testperson + '/' + \
...
\nWenn Sie das Signal während des Experiments entdecken, drücken Sie bitte "1".
...
\nFalls Sie gleich nur das Rauschen wahrnehmen sollten, drücken Sie bitte "2".
\n[Weiter]"""
            self.instruction4 = u"""
...
\nZur Erinnerung:
\nFalls Sie in dem Stimulus das Signal erkennen, drücken Sie bitte "1".
\nFalls Sie das Signal NICHT entdecken können, drücken Sie "2".
...
\nFür das erfolgreiche Lösen ist es wichtig, dass Sie Ihre Aufgabe komplett verstanden haben. Falls Sie noch Fragen haben, stellen Sie diese bitte dem Versuchsleiter.
...
\nDazu drücken Sie, nachdem Sie den ersten Stimulus gesehen haben, "1" falls Sie denken, dass das Signal darin war, oder "2" falls nicht.
\n[Weiter]"""
            self.instruction4 = u"""
Danach wird Ihnen der zweite Stimulus angezeigt und Sie können erneut antworten ob das Signal darin war oder nicht. 
\nDas Programm setzt die zweite Antwort automatisch auf das Gegenteil der ersten Antwort, wenn Sie allerdings "1" oder "2" drücken, überschreiben Sie die zweite Antwort.
